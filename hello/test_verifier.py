...
def main(courses):
    ...
    certs_filename = "certs.json"
    # major_filename = major_name + "_" + str(year)  + ".json"
    filepath = os.path.join(certs_filename)
    with open(filepath, 'r') as f:
        allCerts = json.load("certs_filename")

    totalOutput = []
    for i in allCerts:
        satisfied,courses,major = check_major(allCerts[i],courses)
